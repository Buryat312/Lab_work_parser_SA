...
def get_data(url):
    ...
    for ad in ads:
        ...
        try:
            product_link = url+ ad.find('div', class_='listbox_title').find('a').get('href')
        except:
            product_link = 'Нет линка'
        try:
            category = soup.find('div', class_='product-index').find('div', class_='portlet-title').find('ul', class_='breadcrumb2').find_all('li', itemprop="itemListElement")[-1].text
        except:
            category = 'Другое'
        
        data.append(Kivano(product_name, product_link, category))
...
def main():
    ...
    for url in all_url:
        for i in range(1, 4):
            ...
            # get_data(url_gen)
            product = Products(get_data(url_gen))
